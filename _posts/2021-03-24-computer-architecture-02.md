...
* * *
인스트럭션(instruction)들은 **바이너리 형태**(machine code)로 인코딩 되어 있다. MIPS 인스트럭션들은 32-bit(1 word)형태의 인스트럭션을 가지고 있다. MIPS 인스트럭션은 R-format, I-format, J-format 이런 식으로 표현이 되고, 모든 인스트럭션들은 **규칙성**(regularity)을 위해서 32-bit 의 wide 형태를 가진다.  

### MIPS R-format Instructions
* * *
모든 인스트럭션들은 32 bits 라고 했다. R-format 은 32 bits 를 6 개의 필드(field)로 쪼개어 구성이 된다.
```
+------------+----------+----------+----------+----------+------------+
...
```
각각의 필드에서 필요한 비트의 수도 명시되어 있다. 필드를 쪼갠 이유는 기본적으로 MIPS 라는 하드웨어에서 이런 식으로 인스트럭션을 인코딩 해서 쓰겠다는 규칙을 정해 놓은 것이다.(하드웨어는 정해진 인스트럭션 포맷을 해석하기만 하면 된다.)  

op : operation code (opcode) // add, sub  
rs : **first source** register number // rs, rt, rd 는 각 5 bits 씩이므로  
...
add $t0, $s1, $s2
```
위의 어셈블리 코드는 R-format 에 해당하는 예제이다. 어셈블리 코드로 작성 된 코드가 어떻게 바이너리 형태로 표현 되는지 알아보자.  

첫 번째로는 MIPS green sheet 라는 ISA manual 을 참조해서 **add 인스트럭션**이 R, I, J 포맷 중 **어떤 포맷에 속하는지 판단**을 해야한다.(add 는 R-format)  

그 다음에는 add 인스트럭션 테이블을 살펴본다. add 와 sub 인스트럭션은 다른 OPCODE 와 FUNCT 을 가진다.
```
CORE INSTRUCTION SET                                OPCODE
                                                    /FUNCT
NAME, MNEMONIC   FORMAT    OPERATION(in Verilog)     (Hex)
Add        add     R       R[rd] = R[rs] + R[rt]  (1) 0/20hex // 0 은 add OPCODE 를 뜻한다. 20hex 는 16진수로 표현한 FUNCT
```

마지막으로 위에서 설명한 6 개의 필드로 나눠진 R-format 에 따라 인스트럭션을 인코딩한다.
```
     op          rs         rt         rd         shamt      funct
+------------+----------+----------+----------+----------+------------+
...
위의 표를 하드웨어가 읽을 수 있으려면 바이너리 코드로 변환되어야 한다. 그 과정은 아래와 같다.
...
위의 각 필드에 있는 값을 붙이면 하나의 이진수 덩어리가 된다. 그 덩어리는 곧 16 진수로도 표현할 수 있다.  
```
...
```
MIPS I-format 은 4 가지 필드로 구성되어 있다. I-format 은 하위 16 bits 를 상수나 주소를 표현한다.  

I-format 의 대표적인 예로는 load / store 인스트럭션이 있다.

load (lw) : 메모리에 있는 데이터를 레지스터로 **가져오는** 인스트럭션  
store (sw) : 레지스터에 있는 데이터를 메모리에 **저장**하는 인스트럭션

두 인스트럭션 모두 레지스터와 메모리에 접근해야 하는데, 레지스터에 접근하려면 **레지스터 번호**를 알아야한다. 그리고 메모리에 접근하기 위해서는 **메모리 주소**를 알아야한다.

rt : destination or source register number // 데이터를 메모리로부터 읽어와서 레지스터에 넣어주는 용도
rs + constant or address : 메모리의 어느 위치에서 데이터를 읽어올지 결정하는 용도

**Design Principle 4 : Good design demands good compromises**
- 만약 포맷이 서로 다르면 디코딩을 복잡하게 만든다.
- R-format 과 I-format 은 서로 다른데, 다르다는 의미는 하드웨어가 R 과 I 포맷에 속하는 인스트럭션을 서로 다르게 처리해줘야 한다. // I-format 인데 6 개의 필드로 쪼개면 안됨
- 그러나 MIPS 의 장점은 모든 인스트럭션의 **포맷이 32-bit 로 동일**하다. (x86 과 같은 것은 인스트럭션마다 조금씩 달라서 비용이 많이 든다)
- MIPS 는 앞의 첫 6 bits 는 **Opcode** 를 의미한다. // 6-bit 만 보고 하드웨어가 3 가지 포맷 중에서 어떤 포맷인지 파악할 수 있다.
...
+------------+--------------------------------------------------------+
    6 bits                             26 bits
```
J-format 은 Opcode 6 bits 를 제외한 나머지 26 bits 로 주소를 표현한다.  

j 혹은 jal 인스트럭션이 대표적인데, 어디로 이동해야 할지에 대한 주소를 address 필드에 담고 있다.
...
```
    ...
    +-----------+    | |                       | |
                     ...
                     +---------------------------+
```
컴퓨터는 프로세서(processor) 형태의 연산장치가 있고, 메모리(memory) 형태의 기억장치가 있다. 우리 프로그램은 다 메모리 안에 프로세스로 존재한다. 

- 하드웨어 관점에서는 인스트럭션이나 데이터나 모두 0101 형태의 바이너리 데이터이다.
- 인스트럭션과 데이터는 메모리에 저장되어 있다.
...
* * *
판단을 내리는 인스트럭션들에 대해서 알아보자. C 언어에서 분기문이나 반복문과 관련된 인스트럭션이다.

### Conditional operations
```
beq rs, rt, L1 // branch equal
```
위의 어셈블리 코드는 rs 와 rt 에 있는 값이 **같다**면, L1 이라는 레이블로 점프를 하라는 의미이다.  
if (rs == rt) 를 어셈블리로 변환한 코드이다.

```
bne rs, rt, L1 // branch not euqal
```
위의 어셈블리 코드는 rs 와 rt 에 있는 값이 **같지 않다**면, L1 이라는 레이블로 점프를 하라는 의미이다.  
if (rs != rt) 를 어셈블리로 변환한 코드이다.
...
0x107 |                |/
      +----------------+
```
우리 입장에서는 save[0]에서 save[1]로 갈 때, 그냥 인덱스를 1 만 올려주면 된다. 그러나, 하드웨어 입장에서는 다르다. save 배열은 word 타입의 데이터이기 때문에 하드웨어 입장에서는 메모리 상에서 **4 bytes** 만큼 움직여야 한다. 0x100 ~ 0x103 이라는 주소값을 save[0]이 차지한다. 그래서 save[0]에서 save[1]로 이동시키려면 save[0]의 시작 주소인 0x100 에서 4 를 더해야 0x104, 즉 save[1]의 주소에 도달할 수 있다. 그래서, 우리는 save[**0**]에서 **0** 의 값이 이진수로 0001 이라고 가정하면 save[**1**]의 **1** 은 0001 에서 sll 인스트럭션을 통해 왼쪽으로 두 번 쉬프트한 0100, 즉 4 를 곱한 값인 **4** 이다.  

이 4 를 가진 `$t1` 과 save 배열의 시작 주소값, 즉 **0x100** 을 가진 `$s6` 을 더한 값을 `$t1` 에 저장한다. 그렇다면 우리는 이제 save[1]의 주소값인 **0x104** 를 `$t1` 에 가지게 되었다.  

이제 우리는 lw Opcode 를 통해서 메모리 상에서 save[1]의 주소(`$t1`)인 0x104 로 간 다음에 거기서 0 만큼 떨어진 위치, 즉 **save[1]의 시작 주소**인 **0x104** 에서 1 word 만큼인 0x107 까지 로드해서 `$t0` 라는 레지스터 변수에 save[1]의 **값**을 저장한다.  

그리고 bne 인스트럭션(branch not equal)을 통해서 `$t0` 와 `$s5` 의 값을 비교하여 값이 다르면 **Exit** 레이블로 이동하고, 같으면 j 인스트럭션을 통해 **Loop** 레이블로 이동한다.  

### Basic Blocks
* * *
Basic Block 은 인스트럭션의 집합(sequence of instructions)인데, 브랜치(branch)가 없다.  

그 의미는 어떠한 input 이 있고 그에 따른 output 이 있는데, output 이 여러 개로 갈라질 수 있다.(if, else 에 의해) 그러한 브랜치가 없는 인스트럭션의 집합을 Basic Block 이라고 한다.  
...
* * *
하드웨어 관점에서, 두 값 중 어떠한 것이 더 크고 작은지에 대해 비교하는 것 보다 **같은지 틀린지 체크**하는 것이 훨씬 쉽다. 그래서 beq 와 bne 같은 것을 사용한다.  

즉, 하드웨어 관점에서는 =, != 이 <, >= 와 같은 것들보다 훨씬 빠르게 처리할 수 있다.
